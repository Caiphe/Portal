<div class="form-container editor-field">
    ...
    <form id="create-team" method="POST" action="{{ route('admin.team.update', $team->id) }}" enctype="multipart/form-data" novalidate>
        ...
        <label class="editor-field-label">
            ...
            <input type="text" name="name" value="{{ $team->name }}" id="team-name" class="form-field" placeholder="Enter a name for your team" maxlength="100" autofocus>
        </label>

        <label class="editor-field-label">
            <h3 for="url">Enter team URL *</h3>
            <input type="text" name="url" value="{{ $team->url }}" id="team-url" placeholder="Enter team URL (Eg. https://url.com)" maxlength="100">
        </label>

        <label class="editor-field-label">
            <h3 for="contact">Enter team contact number *</h3>
            <input type="text" name="contact" value="{{ $team->contact }}" id="team-contact" placeholder="Team contact number (e.g +243740000000)" maxlength="16">
        </label>
            ...
            <div class="country-block-container">
                <select id="team-country" name="country" autocomplete="off">
                    ...
                    @foreach($countries as $code => $name)
                        <option value="{{ $code }}" @if($team->country === $code) selected @endif>{{ $name }}</option>
                    @endforeach
                </select>
            </div>
        </label>
        ...
        <label class="editor-field-label">
            ...
            <textarea name="description" id="description" placeholder="Write a short description about your team" maxlength="512">{{ $team->description }}</textarea>
        </label>
